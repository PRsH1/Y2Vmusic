...
- 다운로드 진행률 표시 (대기 중 / 서버 처리 중 / 파일 전송 중 정확한 % — 가짜 진행바 없음)
- 다운로드 완료 알림 (저장한 파일명 표시)
- 인앱 사용법 가이드 (PC/모바일/FAQ 탭 모달)
- 라이트/다크 테마 전환 (시스템 설정 감지 + 수동 토글)
- YouTube Music 차트 탐색 (한국·글로벌·일본 주간 Top 100 + 장르별 선곡 목록 9종)
- 곡 검색 (YouTube 검색 연동)
- 미리듣기 (선택한 트랙 바로 아래 인라인 YouTube 플레이어, ESC로 닫기)
- 다운로드 패널도 선택한 트랙 바로 아래에 열림 (목록을 벗어나지 않음)
- 진행률·완료 알림은 화면 하단에 고정 (목록을 둘러봐도 계속 보임)
- 포맷·품질 선택 기억
- 아티스트·제목 자동 정리 (채널명·"Official MV" 제거, 저장 전 수정 가능) + 파일명 "아티스트 - 제목"
- 정사각형 앨범아트
- 다운로드 기록 (최근 받은 곡, 차트의 "받음" 표시, 중복 안내)
- 구간 자르기 (직접 입력 / 뮤직비디오 비음악 구간 자동 제안 / 미리듣기 위치 찍기, 잘린 끝 0.5초 페이드아웃)
- 주간 차트와 선곡 목록을 구분해 표시 (원본 이름·수집 시각 포함)
...
**방법 A — URL 직접 입력**
1. YouTube URL을 맨 위 입력란에 붙여넣고 검색 버튼 클릭
2. 자동으로 채워진 아티스트·제목 확인 (틀렸으면 수정 — 파일명과 태그에 그대로 들어감)
3. (선택) 구간 자르기: 시작·끝 입력, 비음악 구간 제안이 뜨면 [적용]
4. 포맷과 품질 선택 (다음 방문에도 기억됨)
5. 다운로드 버튼 클릭 → 화면 하단 진행률 확인 → 파일 저장

**방법 B — 차트/검색에서 선택**
1. 탐색 섹션에서 탭(한국 주간 Top 100, K-Pop, 힙합 등) 또는 검색으로 곡 탐색
2. (선택) 썸네일 또는 미리듣기 버튼으로 미리듣기 (트랙 바로 아래에 플레이어가 펼쳐짐)
3. 다운로드 버튼 → 그 트랙 바로 아래에 다운로드 패널이 열림 ("✓ 받음"은 이미 받은 곡)
4. 아티스트·제목·포맷 확인, 필요하면 미리듣기 위치를 찍어 구간 자르기
5. 다운로드 → 받는 동안 목록을 계속 둘러봐도 진행률은 하단에 표시됨

이전에 받은 곡은 "최근 받은 곡"에서 다시 받을 수 있습니다(이 브라우저에만 저장).
...
- **비싼 작업은 동시 실행이 제한**됩니다. 추출·변환은 한 번에 하나만 돌고, 이미 처리 중이면 **순서를 기다렸다 이어서 처리**합니다(최대 3분 대기). 기다리는 동안 진행바에 "대기 중"이라고 표시됩니다. 차트·검색·정보 조회는 별도 풀이라 다운로드 중에도 막히지 않습니다.
- **모든 외부 명령에 시간 제한**이 있습니다. 멈춘 작업은 자식 프로세스까지 함께 정리됩니다.

> 현재 로그인 인증과 요청 수 제한은 없습니다. 링크를 널리 공유하지 마세요.
